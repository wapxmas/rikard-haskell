...
class Box
{
    ...
    public function setUnderlineColor(Color $color)
    {
    ...
    protected function imagettftextunderlined($image,$size,$angle,$x,$y,$color,$fontfile,$text)
    {
        $bbox = imagettftext($image, $size, $angle, $x, $y, $color, $fontfile, $text);
        $subposition = floor(($bbox[1]-$bbox[5]) / 15);
        $thickness = floor(($bbox[1]-$bbox[5]) / 20);
        imagefilledrectangle($image,$bbox[0],$subposition+$bbox[1],$bbox[2],$subposition+
            $bbox[1]+$thickness,$this->underlineColor->getIndex($this->im));
    }
}
